...
namespace QuantLib {
    ...
    */
    ...
    inline Date::Date(Date::serial_type serialNumber)
    ...
    inline Date& Date::operator+=(Date::serial_type days) {
    ...
    inline Date Date::advance(const Date& date, Integer n, TimeUnit units) {
        switch (units) {
          case Days:
            return date + n;
          case Weeks:
    ...
    namespace {
        inline const boost::gregorian::date& serialNumberDateReference() {
...
#define compatibleEnums (   int(boost::date_time::Monday)   +1 == Monday    \
        ...
        inline void advance(ptime& dt, Integer n, TimeUnit units) {
        ...
        inline boost::gregorian::date gregorianDate(Year y, Month m, Day d) {
            QL_REQUIRE(y > 1900 && y < 2200,
                       "year " << y << " out of bound. It must be in [1901,2199]");
            QL_REQUIRE(Integer(m) > 0 && Integer(m) < 13,
                       "month " << Integer(m)
    ...
    inline Date::Date(const ptime& dateTime)
    : dateTime_(dateTime) {}
    ...
    inline Hour Date::hours() const {
        return dateTime_.time_of_day().hours();
    }

    inline Minute Date::minutes() const {
        return dateTime_.time_of_day().minutes();
    }

    inline Second Date::seconds() const {
        return dateTime_.time_of_day().seconds();
    }

    inline Time Date::fractionOfDay() const {
    ...
    inline Time Date::fractionOfSecond() const {
        return dateTime_.time_of_day().fractional_seconds()
            /Real(ticksPerSecond());
    }

    inline Millisecond Date::milliseconds() const {
        return dateTime_.time_of_day().fractional_seconds()
               /(ticksPerSecond()/1000);
    }

    inline Microsecond Date::microseconds() const {
        return (dateTime_.time_of_day().fractional_seconds()
    ...
    inline const ptime& Date::dateTime() const { return dateTime_; }

    inline Date& Date::operator+=(Date::serial_type d) {
    ...
    inline Date Date::localDateTime() {
        return Date(boost::posix_time::microsec_clock::local_time());
    }

    inline Date Date::universalDateTime() {
    ...
    inline Date::serial_type Date::minimumSerialNumber() {
        return 367;       // Jan 1st, 1901
    }

    inline Date::serial_type Date::maximumSerialNumber() {
        return 109574;    // Dec 31st, 2199
    }

    inline void Date::checkSerialNumber(Date::serial_type serialNumber) {
    ...
    inline Date Date::nthWeekday(Size nth, Weekday dayOfWeek,
                          Month m, Year y) {
        QL_REQUIRE(nth>0,
    ...
    inline std::ostream& operator<<(std::ostream& out, Month m) {
        switch (m) {
          case January:
            return out << "January";
          case February:
    ...
    namespace io {
        inline detail::short_date_holder short_date(const Date& d) {
            return detail::short_date_holder(d);
        }

        inline detail::long_date_holder long_date(const Date& d) {
            return detail::long_date_holder(d);
        }

        inline detail::iso_date_holder iso_date(const Date& d) {
            return detail::iso_date_holder(d);
        }

        inline detail::formatted_date_holder formatted_date(const Date& d,
                                                     const std::string& f) {
            return detail::formatted_date_holder(d, f);
        }

#ifdef QL_HIGH_RESOLUTION_DATE
        inline detail::iso_datetime_holder iso_datetime(const Date& d) {
