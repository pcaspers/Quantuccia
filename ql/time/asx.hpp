...
namespace QuantLib {
    ...
    inline bool ASX::isASXdate(const Date& date, bool mainCycle) {
    ...
    inline bool ASX::isASXcode(const std::string& in, bool mainCycle) {
        if (in.length() != 2)
            return false;
    ...
    inline std::string ASX::code(const Date& date) {
    ...
    inline Date ASX::date(const std::string& asxCode,
                   const Date& refDate) {
...
//        Year y = boost::lexical_cast<Year>(); // lexical_cast causes compilation errors with x64
    ...
    inline Date ASX::nextDate(const Date& date, bool mainCycle) {
    ...
    inline Date ASX::nextDate(const std::string& ASXcode,
                       bool mainCycle,
    ...
    inline std::string ASX::nextCode(const Date& d,
                              bool mainCycle) {
        Date date = nextDate(d, mainCycle);
        return code(date);
    }

    inline std::string ASX::nextCode(const std::string& asxCode,
                              bool mainCycle,
                              const Date& referenceDate) {
