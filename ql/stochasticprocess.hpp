...
namespace QuantLib {
    ...
    */
    ...
    inline StochasticProcess::StochasticProcess() {}

    inline StochasticProcess::StochasticProcess(
                                const boost::shared_ptr<discretization>& disc)
    : discretization_(disc) {}

    inline Size StochasticProcess::factors() const {
        return size();
    }

    inline Disposable<Array> StochasticProcess::expectation(Time t0,
                                                     const Array& x0,
                                                     Time dt) const {
        return apply(x0, discretization_->drift(*this, t0, x0, dt));
    }

    inline Disposable<Matrix> StochasticProcess::stdDeviation(Time t0,
                                                       const Array& x0,
                                                       Time dt) const {
        return discretization_->diffusion(*this, t0, x0, dt);
    }

    inline Disposable<Matrix> StochasticProcess::covariance(Time t0,
                                                     const Array& x0,
                                                     Time dt) const {
        return discretization_->covariance(*this, t0, x0, dt);
    }

    inline Disposable<Array> StochasticProcess::evolve(
                                             Time t0, const Array& x0,
                                             Time dt, const Array& dw) const {
        return apply(expectation(t0,x0,dt), stdDeviation(t0,x0,dt)*dw);
    }

    inline Disposable<Array> StochasticProcess::apply(const Array& x0,
                                               const Array& dx) const {
        return x0 + dx;
    }

    inline Time StochasticProcess::time(const Date& ) const {
        QL_FAIL("date/time conversion not supported");
    }

    inline void StochasticProcess::update() {
        notifyObservers();
    }
    ...
    inline StochasticProcess1D::StochasticProcess1D() {}

    inline StochasticProcess1D::StochasticProcess1D(
                                const boost::shared_ptr<discretization>& disc)
    : discretization_(disc) {}

    inline Real StochasticProcess1D::expectation(Time t0, Real x0, Time dt) const {
        return apply(x0, discretization_->drift(*this, t0, x0, dt));
    }

    inline Real StochasticProcess1D::stdDeviation(Time t0, Real x0, Time dt) const {
        return discretization_->diffusion(*this, t0, x0, dt);
    }

    inline Real StochasticProcess1D::variance(Time t0, Real x0, Time dt) const {
        return discretization_->variance(*this, t0, x0, dt);
    }

    inline Real StochasticProcess1D::evolve(Time t0, Real x0,
                                     Time dt, Real dw) const {
        return apply(expectation(t0,x0,dt), stdDeviation(t0,x0,dt)*dw);
    }

    inline Real StochasticProcess1D::apply(Real x0, Real dx) const {
        return x0 + dx;
    }
