...
namespace {
    ...
    inline std::vector<Real> xRange(Real start, Real finish, Size points) {
    ...
    inline std::vector<Real> gaussian(const std::vector<Real>& x) {
    ...
    inline std::vector<Real> parabolic(const std::vector<Real>& x) {
        std::vector<Real> y(x.size());
        for (Size i=0; i<x.size(); i++)
            y[i] = -x[i]*x[i];
        return y;
    ...
    inline void check1stDerivativeValue(const char* type,
                                 const CubicInterpolation& cubic,
                                 Real x,
                                 Real value) {
    ...
    inline void check2ndDerivativeValue(const char* type,
                                 const CubicInterpolation& cubic,
                                 Real x,
                                 Real value) {
    ...
    inline void checkNotAKnotCondition(const char* type,
                                const CubicInterpolation& cubic) {
        Real tolerance = 1.0e-14;
    ...
    inline void checkSymmetry(const char* type,
                       const CubicInterpolation& cubic,
                       Real xMin) {
    ...
    inline Real multif(Real s, Real t, Real u, Real v, Real w) {
        return std::sqrt(s * std::sinh(std::log(t)) +
                         std::exp(std::sin(u) * std::sin(3 * v)) +
                         std::sinh(std::log(v * w)));
    }

    inline Real epanechnikovKernel(Real u){
